function [out, o_lightR, o_lightG, o_lightB]=HDM_OFT_LightCalibration...
...
subplot(2,2,4)
plot...
    (OFT_SpectrumImagePixelColumnIndex,OFT_Spectrum_MeanOfRows(1,:,3),'b',...
    OFT_SpectrumImagePixelColumnIndex,OFT_Spectrum_MeanOfRows(1,:,2),'g',...
    OFT_SpectrumImagePixelColumnIndex,OFT_Spectrum_MeanOfRows(1,:,1),'r');
legend({'b','g','r'})
xlabel('horicontal pixel index')
ylabel('mean pixel value of all lines')

o_lightR = OFT_Spectrum_MeanOfRows(1,:,1);
...
plot...
    (l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsBlue_nmSamples,'b',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsGreen_nmSamples,'g',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsRed_nmSamples,'r',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsBlue_nmSamplesCam,':b',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsGreen_nmSamplesCam,':g',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsRed_nmSamplesCam,':r');
...
plot...
    ...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsGreen_nmSamples,'g',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsRed_nmSamples,'r',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsBlue_nmSamplesLight,':b',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsGreen_nmSamplesLight,':g',...
    l_IntenityAgainstWavelength(1,:),OFT_Spectrum_MeanOfRowsRed_nmSamplesLight,':r')
...
OFT_Out_SpectralResponse(OFT_Out_SpectralResponse~=OFT_Out_SpectralResponse)=0;

OFT_RICD_Response = HDM_OFT_GetSpectralResponse('RICD');
...
l_linCombOfStdSensitiviesM = mldivide(OFT_CIEStandardObserver_SpectralCurves(2 : 4, :)', ...
                                        OFT_Out_SpectralResponse(2 : 4, :)');
                                    
l_linCombOfStdSensitivies = (OFT_CIEStandardObserver_SpectralCurves(2 : 4, :)' * l_linCombOfStdSensitiviesM)';

l_RMSE_R = sqrt(mean((l_linCombOfStdSensitivies(1, :)' - OFT_Out_SpectralResponse(2, :)').^2));
l_RMSE_G = sqrt(mean((l_linCombOfStdSensitivies(2, :)' - OFT_Out_SpectralResponse(3, :)').^2));
l_RMSE_B = sqrt(mean((l_linCombOfStdSensitivies(3, :)' - OFT_Out_SpectralResponse(4, :)').^2));

figure ('Name','final response')
plot...
    (OFT_Out_SpectralResponse(1,:),OFT_Out_SpectralResponse(4,:),'b',...
    OFT_Out_SpectralResponse(1,:),OFT_Out_SpectralResponse(3,:),'g',...
    OFT_Out_SpectralResponse(1,:),OFT_Out_SpectralResponse(2,:),'r',...
...%     OFT_RICD_Response(1,:), OFT_RICD_Response(4,:), ...
...%     OFT_RICD_Response(1,:), OFT_RICD_Response(3,:), ...
...%     OFT_RICD_Response(1,:), OFT_RICD_Response(2,:), ...
...
ylabel('Green Normalized Response')
grid on
grid minor
